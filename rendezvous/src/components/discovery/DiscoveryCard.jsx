...
const DiscoveryCard = ({ user, onSwipe }) => {
  ...
  const photos = user.PHOTOS || [user.PHOTO_URL, "https://images.unsplash.com/photo-1524504388940-b1c1722653e1", "https://images.unsplash.com/photo-1517841905240-472988babdf9"];
  ...
  return (
    <div className="relative w-[380px] h-[600px] flex flex-col items-center font-sans">
        ...
        <div className="relative w-full h-full bg-gray-100">
          ...
          {!showInfo && (
            <div className="absolute bottom-0 inset-x-0 h-1/2 bg-gradient-to-t from-black/80 via-black/20 to-transparent p-6 text-white flex flex-col justify-end pointer-events-none">
              ...
              <p className="text-sm flex items-center gap-1 mt-1 opacity-90"><MapPin size={14} /> {user.CITY}</p>
            ...
            {showInfo && (
              ...
              >
                ...
                <div className="p-6 space-y-8 pb-10">
                  <section>
                    <h4 className="text-gray-900 font-bold mb-3">내가 찾는 관계</h4>
                    <div className="bg-pink-50/50 p-4 rounded-2xl border border-pink-100 flex items-center gap-3">
                      <span className="text-2xl">🌹</span>
                      <span className="font-bold text-pink-600">진지한 연애</span>
                    </div>
                  </section>

                  <section>
                    <h4 className="text-gray-900 font-bold mb-2">자기소개</h4>
                    <p className="text-gray-600 text-[15px] leading-relaxed">안녕하세요! {user.NICKNAME}입니다. 좋은 인연 만들어가요!</p>
                  </section>
                  ...
                  <section>
                    ...
                    <div className="space-y-4">
                      {[
                        { label: "애정표현 스타일", val: "배려심 깊은 행동", icon: "☘️" },
                        { label: "학력", val: "대학교 졸업", icon: "🎓" },
                        { label: "연락 스타일", val: "카톡 자주 하는 편", icon: "💌" },
                  ...
                  <section>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {["언어 교환", "명상", "여행", "코미디", "동네 산책"].map(tag => (
                        <span key={tag} className="px-3 py-1.5 bg-gray-50 rounded-full text-xs font-bold text-gray-600 border border-gray-100">{tag}</span>
                      ))}
                    </div>
                  </section>
      ...
      <div className="absolute bottom-0 flex items-center gap-5 z-20">
        <button onClick={() => onSwipe("left")} className="w-16 h-16 flex items-center justify-center bg-white rounded-full shadow-xl text-gray-300 border border-gray-50 hover:scale-110 active:scale-90 transition-all">
          <X size={35} strokeWidth={3} />
        </button>
        <button onClick={() => setShowInfo(!showInfo)} className={`w-12 h-12 flex items-center justify-center rounded-full shadow-lg transition-all ${showInfo ? "bg-black text-white" : "bg-white text-gray-300 border border-gray-50"}`}>
          <ChevronDown size={28} className={showInfo ? "rotate-180" : ""} />
        </button>
        <button onClick={() => onSwipe("right")} className="w-16 h-16 flex items-center justify-center bg-white rounded-full shadow-xl text-pink-500 border border-gray-50 hover:scale-110 active:scale-90 transition-all">
